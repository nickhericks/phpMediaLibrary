...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($error_message)) {
      ...
      $mail->Username = "nickhericks@gmail.com";
      //Password to use for SMTP authentication
      $mail->Password = "geooefknsqlgtbnm";

          //It's important not to use the submitter's address as the from address as it's forgery,
          //which will cause your messages to fail SPF checks.
          //Use an address in your own domain as the from address, put the submitter's address in a reply-to
          $mail->setFrom('hello@nickhericks.com', $name);
          $mail->addReplyTo($email, $name);
          $mail->addAddress('hello@nickhericks.com', 'Nick Hericks');
          $mail->Subject = 'Library suggestion from ' . $name;
          $mail->Body = $email_body;
          if ($mail->send()) {
            // Brings user to page their on, but includes status=thanks as a $_GET variable
      ...
      <table>
        ...
        <tr>
          ...
          <td><select id="category" name="category" />
            ...
            if(isset($category) && $category == 'Movies') { echo ' selected';} ?> >Movie</option>
            <option value="Music" <?php
            if(isset($category) && $category == 'Music') { echo ' selected';} ?> >Music</option>
          </td>
        </tr>
        <tr>
